	...
	if (Killer)
	{
		...
		if (Killer->GetController())
		{
			if (AGDKPlayerController* KC = Cast<AGDKPlayerController>(Killer->GetController()))
			{
				...
				{
					KC->InformOfKill(PC->GetPlayerName());
				}
			}
		}
	}
...
void AGDKPlayerController::InformOfKill_Implementation(const FString& VictimName)
{
	KillNotification.Broadcast(VictimName);
}
void AGDKPlayerController::InformOfDeath_Implementation(const FString& KillerName)
{
	KilledNotification.Broadcast(KillerName);
